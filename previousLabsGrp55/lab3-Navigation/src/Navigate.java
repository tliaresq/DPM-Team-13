...
public class Navigate extends Thread {
	...
	public void run() {
		//limitation: if a destination can't be reached, the nxt will turn around it forever
	}
	...
	private static void follow(double x, double y, Odometer odometer) {
		...
		//limitation: doesn't see obstacle right ahead if already following an obstacle;
		sensorMotor.rotate(-80, true);
		leftMotor.rotate(convertAngle(leftRadius, width, 90), true);
		rightMotor.rotate(-convertAngle(rightRadius, width, 90), false);
		delta = deltaAngle();
		
		//follow obstacle while the theta isn't within 5 degrees of the angle needed to reach destination 
	...
	//calculate the difference in angle between the current angle and the angle pointing to direction
